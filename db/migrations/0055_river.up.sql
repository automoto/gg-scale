...
-- ggscale_app uses River's tables at runtime (River's client runs under the
-- app role via the pool's SET ROLE). The tables stay owned by the migration
-- role and are deliberately NOT row-level-security scoped: they are
-- cross-tenant infrastructure, not tenant data.
GRANT SELECT, INSERT, UPDATE, DELETE
    ON river_job, river_leader, river_queue, river_client, river_client_queue
    TO ggscale_app;
GRANT USAGE, SELECT ON SEQUENCE river_job_id_seq TO ggscale_app;
