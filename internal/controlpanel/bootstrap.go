...
func emitBootstrapToken(token, tokenFile string, logger *slog.Logger, w io.Writer) error {
	if tokenFile != "" {
		if err := os.WriteFile(tokenFile, []byte(token+"\n"), 0o600); err != nil {
			return fmt.Errorf("write control panel bootstrap token file: %w", err)
		}
		logger.Warn("control panel bootstrap token written to file",
			"path", "/v1/control-panel/setup",
			"token_file", tokenFile)
		return nil
	}
	logger.Warn("control panel bootstrap token written to stderr — set CONTROL_PANEL_BOOTSTRAP_TOKEN_FILE to avoid this",
		"path", "/v1/control-panel/setup")
	_, _ = fmt.Fprintf(w, "\n  ggscale bootstrap token: %s\n\n", token)
	return nil
}

// LoadBootstrap builds a first-run bootstrap guard from the control panel users
...
func LoadBootstrap(ctx context.Context, pool *db.Pool, tokenFile string, logger *slog.Logger) (*Bootstrap, error) {
	...
	if err := pool.BootstrapQ(ctx, func(tx pgx.Tx) error {
		var err error
		count, err = sqlcgen.New(tx).CountControlPanelUsers(ctx)
		return err
	}); err != nil {
		return nil, fmt.Errorf("control panel bootstrap count users: %w", err)
	}
	if count > 0 {
		return DisabledBootstrap(), nil
	}
