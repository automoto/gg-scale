...
func (h *Handler) updatePassword(w http.ResponseWriter, r *http.Request) {
	...
	if len(next) < minControlPanelPassLen {
	...
	if err := h.pool.BootstrapQ(r.Context(), func(tx pgx.Tx) error {
		q := sqlcgen.New(tx)
		if err := q.UpdateControlPanelPassword(r.Context(), sqlcgen.UpdateControlPanelPasswordParams{
			ID:           session.User.ID,
			PasswordHash: hash,
		}); err != nil {
			return err
		...
		return auditlog.WritePlatform(r.Context(), tx, session.User.ID, "control_panel.password_change", "", nil)
	}); err != nil {
