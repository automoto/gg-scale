...
const (
	verifyPendingCookieName = "ggscale_control_panel_verify"
...
func (h *Handler) startVerification(ctx context.Context, userID int64, email string) error {
	state, err := h.fetchControlPanelVerifyState(ctx, userID)
	if err != nil {
		return err
	}
	...
	err = h.pool.BootstrapQ(ctx, func(tx pgx.Tx) error {
		return sqlcgen.New(tx).SetControlPanelUserVerificationCode(ctx, sqlcgen.SetControlPanelUserVerificationCodeParams{
			ID:        userID,
			CodeHash:  codeHash,
			CodeSalt:  salt,
...
func (h *Handler) fetchControlPanelVerifyState(ctx context.Context, userID int64) (sqlcgen.GetControlPanelUserVerificationStateRow, error) {
	var row sqlcgen.GetControlPanelUserVerificationStateRow
	err := h.pool.BootstrapQ(ctx, func(tx pgx.Tx) error {
		var err error
		row, err = sqlcgen.New(tx).GetControlPanelUserVerificationState(ctx, userID)
		return err
	})
	return row, err
}
...
func (h *Handler) confirmVerification(ctx context.Context, userID int64, code string) error {
	...
	var reserved sqlcgen.ReserveControlPanelVerifyAttemptRow
	resErr := h.pool.BootstrapQ(ctx, func(tx pgx.Tx) error {
		var qerr error
		reserved, qerr = sqlcgen.New(tx).ReserveControlPanelVerifyAttempt(ctx, sqlcgen.ReserveControlPanelVerifyAttemptParams{
			ID:          userID,
	...
	if verifycode.LifetimeExhausted(int(reserved.EmailVerificationLifetimeAttempts)) {
		...
		_ = h.pool.BootstrapQ(ctx, func(tx pgx.Tx) error {
			return sqlcgen.New(tx).LockControlPanelUserVerification(ctx, sqlcgen.LockControlPanelUserVerificationParams{
	...
	return h.pool.BootstrapQ(ctx, func(tx pgx.Tx) error {
		return sqlcgen.New(tx).MarkControlPanelUserVerified(ctx, userID)
	})
}
...
func (h *Handler) verifyHandler(w http.ResponseWriter, r *http.Request) {
	...
	case errors.Is(err, errAlreadyVerified):
		...
		http.Redirect(w, r, "/v1/control-panel/login?flash="+url.QueryEscape("Your email is already verified. Please sign in."), http.StatusSeeOther)
		return
	...
	case errors.Is(err, errVerifyAccountLocked):
		...
		return
	case err != nil:
		slog.ErrorContext(r.Context(), "control panel verify confirm", "err", err)
	...
	h.clearVerifyPendingCookie(w)
	h.finishLogin(w, r, controlPanelUser{ID: p.UserID, Email: p.Email})
}
...
func (h *Handler) verifyResendHandler(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, "/v1/control-panel/verify?flash="+url.QueryEscape("A new code was sent."), http.StatusSeeOther)
}
