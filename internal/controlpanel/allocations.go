...
func (h *Handler) allocationsAllocateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !h.requireControlPanelAllocationMutation(w, r, tenantID, projectID, rbac.ActionAllocate) {
		return
	}
...
func pageParam(r *http.Request) int {
	...
	if p > maxControlPanelPage {
		return maxControlPanelPage
	}
	return p
}

func controlPanelPageOffset(page, pageSize int) int32 {
	offset := (page - 1) * pageSize
	if offset <= 0 {
		return 0
	}
	// #nosec G115 -- pageParam clamps page to maxControlPanelPage and control panel page sizes are small constants.
	return int32(offset)
}

func controlPanelPageLimit(pageSize int) int32 {
	// #nosec G115 -- control panel page sizes are small constants and +1 is the LIMIT+1 sentinel.
