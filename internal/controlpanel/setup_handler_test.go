...
func TestSetupTokenPage_ShowsFilePathFromBootstrap(t *testing.T) {
	...
	h.setupTokenPage(w, httptest.NewRequest(http.MethodGet, "/v1/control-panel/setup", nil))

	assert.Equal(t, http.StatusOK, w.Code)
	assert.Contains(t, w.Body.String(), "/tmp/p")
	assert.Contains(t, w.Body.String(), `action="/v1/control-panel/setup/token"`)
}
...
func TestVerifySetupToken_RejectsWrongToken(t *testing.T) {
	...
	body := w.Body.String()
	assert.Contains(t, body, `action="/v1/control-panel/setup/token"`)
...
func TestVerifySetupToken_RendersStep2OnGoodToken(t *testing.T) {
	...
	assert.Contains(t, body, `action="/v1/control-panel/setup"`)
}
