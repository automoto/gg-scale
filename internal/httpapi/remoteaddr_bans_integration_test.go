...
func TestTenantBan_blocks_login(t *testing.T) {
	...
	require.NoError(t, c.bootstrapPool.QueryRow(context.Background(),
		`SELECT id, tenant_id FROM end_users WHERE email = 'banme@example.com'`).Scan(&endUserID, &tenantID))
	accID := linkEndUserAccountReturningID(t, c, endUserID)
	...
	_, err = c.bootstrapPool.Exec(context.Background(),
		`UPDATE end_users SET session_epoch = session_epoch + 1 WHERE player_account_id = $1 AND tenant_id = $2`, accID, tenantID)
	require.NoError(t, err)
...
func linkEndUserAccountReturningID(t *testing.T, c *cluster, endUserID int64) string {
	t.Helper()
	var accID string
	require.NoError(t, c.bootstrapPool.QueryRow(context.Background(),
		`INSERT INTO player_accounts (email, password_hash, email_verified_at)
		 VALUES ($1, '\x00'::bytea, now()) RETURNING id`,
		fmt.Sprintf("acct-%d@example.com", endUserID)).Scan(&accID))
	_, err := c.bootstrapPool.Exec(context.Background(),
