...
var (
	errInvalidLimit    = errors.New("dashboard: rate and burst must be finite non-negative numbers")
	errExceedsCap      = errors.New("dashboard: per-project quota exceeds the tenant cap")
	errIncompleteLimit = errors.New("dashboard: rate and burst must both be positive (or both blank to clear)")
)
...
func (h *Handler) setTenantAPIOverride(ctx context.Context, actorID, tenantID int64, rate, burst float64) error {
	...
	err := h.pool.BootstrapQ(ctx, func(tx pgx.Tx) error {
		...
		if rate == 0 && burst == 0 {
			...
			return auditlog.WritePlatform(ctx, tx, actorID, "dashboard.rate_limit.clear", strconv.FormatInt(tenantID, 10),
		...
		return auditlog.WritePlatform(ctx, tx, actorID, "dashboard.rate_limit.set", strconv.FormatInt(tenantID, 10),
...
func (h *Handler) setProjectInviteOverride(ctx context.Context, actorID, tenantID, projectID int64, inviterPerHour, domainPerDay float64) error {
	...
	err := h.pool.BootstrapQ(ctx, func(tx pgx.Tx) error {
		...
		return auditlog.WritePlatform(ctx, tx, actorID, "dashboard.rate_limit.set", strconv.FormatInt(projectID, 10),
