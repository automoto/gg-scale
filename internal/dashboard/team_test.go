...
func TestTenantTeamPath(t *testing.T) {
	assert.Equal(t, "/v1/dashboard/tenants/42/team", tenantTeamPath(42))
}
...
func TestInviteAcceptURL_format(t *testing.T) {
	...
	assert.Equal(t, "https://app.example.com/v1/dashboard/invite/accept?code=abc-XYZ_123", got)
}

func TestInviteAcceptURL_empty_base(t *testing.T) {
	h := &Handler{cfg: Config{}}
	got := h.inviteAcceptURL("zzz")
	assert.Equal(t, "/v1/dashboard/invite/accept?code=zzz", got)
}
