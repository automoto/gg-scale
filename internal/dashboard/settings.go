...
// safeReturnPath returns raw when it is a same-origin dashboard-relative path
...
// "?flash="), dot segments, and anything outside /v1/dashboard, so the
...
func safeReturnPath(raw, fallback string) string {
	...
	if raw != pathDashboard && !strings.HasPrefix(raw, pathDashboard+"/") {
		return fallback
	}
	u, err := url.Parse(raw)
