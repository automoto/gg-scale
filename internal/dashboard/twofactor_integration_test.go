...
const (
	...
	tfLoginPath    = "/v1/dashboard/login"
	tfChallenge    = "/v1/dashboard/login/2fa"
)
...
func jarCookie(t *testing.T, c *http.Client, base, name string) *http.Cookie {
	t.Helper()
	u, err := url.Parse(base + pathDashboard)
	require.NoError(t, err)
...
func resetTOTPStep(t *testing.T, raw *pgxpool.Pool, userID int64) {
	...
	_, err := raw.Exec(context.Background(),
		`UPDATE dashboard_user_totp SET last_used_step = 0 WHERE dashboard_user_id = $1`, userID)
	require.NoError(t, err)
}
...
func latestCSRF(t *testing.T, raw *pgxpool.Pool, userID int64) string {
	...
	require.NoError(t, raw.QueryRow(context.Background(),
		`SELECT csrf_secret FROM dashboard_sessions
		 WHERE dashboard_user_id = $1 AND revoked_at IS NULL
...
func TestDashboardTwoFactor_fullFlow(t *testing.T) {
	...
	root := chi.NewRouter()
	root.Mount(pathDashboard, New(Deps{Pool: pool, Config: Config{Mount: true}, Mailer: noopMailer, TwoFactor: cipher}))
	...
	// Without 2FA the login lands straight on the dashboard with a session.
	resp, _ := tfPostForm(t, alice, srv.URL+tfLoginPath, loginForm("op@example.com"))
	require.Equal(t, http.StatusSeeOther, resp.StatusCode)
	require.Equal(t, pathDashboard, resp.Header.Get("Location"))
	...
	resp, body := tfPostForm(t, alice, srv.URL+"/v1/dashboard/account/2fa/setup", url.Values{"_csrf": {csrf}})
	...
	resp, body = tfPostForm(t, alice, srv.URL+"/v1/dashboard/account/2fa/confirm",
	...
	resp, _ = tfPostForm(t, bob, srv.URL+tfLoginPath, loginForm("op@example.com"))
	require.Equal(t, http.StatusSeeOther, resp.StatusCode)
	assert.Equal(t, pathDashboard, resp.Header.Get("Location"))
	...
	_, err = raw.Exec(context.Background(),
		`UPDATE dashboard_user_totp SET locked_until = now() - interval '1 second' WHERE dashboard_user_id = $1`, userID)
	require.NoError(t, err)
	resp, _ = tfPostForm(t, dave, srv.URL+tfChallenge, url.Values{"code": {totpNow(t, secret)}})
	...
	t.Run("verify_path_cannot_bypass_challenge", func(t *testing.T) {
		...
		_, err := raw.Exec(context.Background(),
			`UPDATE dashboard_users SET email_verified_at = NULL WHERE id = $1`, verifyUserID)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		_, err = raw.Exec(context.Background(),
			`INSERT INTO dashboard_user_totp (dashboard_user_id, secret_enc, confirmed_at) VALUES ($1, $2, now())`,
			verifyUserID, enc)
		require.NoError(t, err)
		...
		require.Equal(t, http.StatusSeeOther, resp.StatusCode)
		require.Equal(t, "/v1/dashboard/verify", resp.Header.Get("Location"))
		...
		resp, _ = tfPostForm(t, erin, srv.URL+"/v1/dashboard/verify", url.Values{"code": {"123456"}})
		require.Equal(t, http.StatusSeeOther, resp.StatusCode)
		assert.Equal(t, tfChallenge, resp.Header.Get("Location"), "verified 2FA user must land on the challenge, not the dashboard")
	...
	t.Run("nil_cipher_fails_closed_for_enrolled_user", func(t *testing.T) {
		bare := chi.NewRouter()
		bare.Mount(pathDashboard, New(Deps{Pool: pool, Config: Config{Mount: true}, Mailer: noopMailer}))
		...
		require.Equal(t, pathDashboard, resp.Header.Get("Location"))
		_ = plainID
		pageResp, err := grace.Get(bareSrv.URL + pathDashboardAccount)
		require.NoError(t, err)
	...
	t.Run("disable_restores_plain_login", func(t *testing.T) {
		csrf := latestCSRF(t, raw, userID)
		resetTOTPStep(t, raw, userID)
		resp, body := tfPostForm(t, dave, srv.URL+"/v1/dashboard/account/2fa/disable", url.Values{
			"_csrf":            {csrf},
...
// TestDashboardTwoFactor_replayedValidCodeDoesNotLock guards the fix for the
...
func TestDashboardTwoFactor_replayedValidCodeDoesNotLock(t *testing.T) {
	pool, raw := startTwoFactorDB(t)
	cipher, err := twofactor.NewCipher(testTwoFactorHexKey)
	require.NoError(t, err)
	noopMailer, err := mailer.New("noop", "", "", "", "noreply@test", "off")
	...
	require.Equal(t, http.StatusSeeOther, resp.StatusCode)
	csrf := latestCSRF(t, raw, userID)
	resp, body := tfPostForm(t, admin, srv.URL+"/v1/dashboard/account/2fa/setup", url.Values{"_csrf": {csrf}})
	require.Equal(t, http.StatusOK, resp.StatusCode)
	secretMatch := regexp.MustCompile(`<code>([A-Z2-7 ]+)</code>`).FindStringSubmatch(body)
	require.Len(t, secretMatch, 2)
	secret := strings.ReplaceAll(secretMatch[1], " ", "")
	resp, _ = tfPostForm(t, admin, srv.URL+"/v1/dashboard/account/2fa/confirm",
		url.Values{"_csrf": {csrf}, "code": {totpNow(t, secret)}})
	require.Equal(t, http.StatusOK, resp.StatusCode)
	...
	_, err = raw.Exec(context.Background(),
		`UPDATE dashboard_user_totp SET last_used_step = $1 WHERE dashboard_user_id = $2`,
