...
func TestSafeReturnPath(t *testing.T) {
	const fallback = "/v1/dashboard/tenants/1/projects"
	cases := []struct {
		raw  string
		want string
	}{
		{"/v1/dashboard/tenants/5/settings", "/v1/dashboard/tenants/5/settings"},
		{"/v1/dashboard", "/v1/dashboard"},
		{"/v1/dashboard/tenants/5/projects/9/settings", "/v1/dashboard/tenants/5/projects/9/settings"},
		{"", fallback},
		{"http://evil.com/x", fallback},
		{"//evil.com", fallback},
		{"/etc/passwd", fallback},
		{"javascript:alert(1)", fallback},
		{"/v1/dashboardX/evil", fallback},
		// Callers append "?flash=", so a raw query or fragment would corrupt
		// the redirect URL.
		{"/v1/dashboard/tenants/5/settings?x=1", fallback},
		{"/v1/dashboard/tenants/5/settings#frag", fallback},
		// Dot segments would let the browser normalize past /v1/dashboard.
		{"/v1/dashboard/../../metrics", fallback},
		{"/v1/dashboard/./evil", fallback},
	}
	for _, c := range cases {
...
func TestTenantSettingsPage_renders_editable_forms_with_return_path(t *testing.T) {
	...
	assert.Contains(t, html, "/v1/dashboard/tenants/3/public-joining")
	assert.Contains(t, html, "Disable for all projects")
	// API limit form is present for platform admins.
	assert.Contains(t, html, "/v1/dashboard/tenants/3/rate-limits/api")
	assert.Contains(t, html, "Save API limit")
	// Both forms carry a redirect_to back to the settings page.
	assert.Contains(t, html, `name="redirect_to" value="/v1/dashboard/tenants/3/settings"`)
}
...
func TestProjectSettingsPage_shows_effective_join_and_forms(t *testing.T) {
	...
	assert.Contains(t, html, "/v1/dashboard/tenants/3/projects/8/public-joining")
	assert.Contains(t, html, "/v1/dashboard/tenants/3/rate-limits/projects/8/invites")
	assert.Contains(t, html, `name="redirect_to" value="/v1/dashboard/tenants/3/projects/8/settings"`)
}
